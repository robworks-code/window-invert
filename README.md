...
Run `WindowInvert-Setup-<version>.exe`. It installs for the
current user only, so there is no administrator prompt, and
adds Window Invert to the Start menu and to Settings > Apps.
The setup wizard offers to start it at sign-in; the same switch
is in the tray menu afterwards. Uninstall from Settings > Apps.

The installer is not code-signed, so SmartScreen shows an
"unrecognized app" warning the first time. Choose "More info"
and then "Run anyway".
...
The installer needs Inno Setup 6 as well
(`winget install --id JRSoftware.InnoSetup -e`). Then:

```
powershell -File build/release.ps1
```

That runs the tests, publishes the app, and writes
`dist/WindowInvert-Setup-<version>.exe`. The version comes from
`src/WindowInvert.App/WindowInvert.App.csproj`.

The icon is generated by `assets/make_icon.py` (needs Pillow);
run it after changing the drawing and commit the resulting
`.ico`.
